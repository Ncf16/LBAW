...
function listView(){
	...
	if($type == 0){
		...
		$course = getCourseIDByName($_POST['course']);//code
		if(!$course){
			...
			$data['nbUnits'] = intval(countUnitOccurrencesCourse($course['code'])['total']);
			if(isset($_POST['nbUnits'])){
				if(intval($_POST['nbUnits']) != $data['nbUnits']){
					$nbPages = ceil($data['nbUnits'] / $itemsPerPage);
					$pageNumber = max(min($nbPages,$pageNumber),1);
			...
			$data['nbUnits'] = intval(countUnitOccurrencesCourseYear($course['code'],$year[0])['total']);
			if(isset($_POST['nbUnits'])){
				if(intval($_POST['nbUnits']) != $data['nbUnits']){
					$nbPages = ceil($data['nbUnits'] / $itemsPerPage);
					$pageNumber = max(min($nbPages,$pageNumber),1);
...
function deleteView(){
	...
	if($data['success'] == 'Success'){
		...
		if($type == 0){
			...
			$course =  getCourseIDByName($_POST['course']);
			if(!$course){
				$_SESSION['form_values'] = $_POST;
				$_SESSION['error_messages'][] = 'Couldn\'t find a course with that name';
				exit;
